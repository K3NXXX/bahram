...
export const Auth: React.FC = () => {
  ...
  const { register: registerRegistration, handleSubmit: handleSubmitRegistration } = registrationForm;
  const { register: registerLogin, handleSubmit: handleSubmitLogin } = loginForm;

  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const registerSubmitHandler: SubmitHandler<registerData> = (data: registerData) => {
    ...
    <div className={style.root}>
      <div className={style.main}>
        ...
        <div className={style.signup}>
          <form onSubmit={handleSubmitRegistration(registerSubmitHandler)}>
            <label htmlFor={style.chk}>Sign up</label>

            <input
              {...registerRegistration("username", {
                minLength: { value: 5, message: "Username minimum 5 characters" },
                required: true,
              })}
            ...
            <input
              {...registerRegistration("password", {
                minLength: { value: 5, message: "Password minimum 5 characters" },
                required: true,
              })}
